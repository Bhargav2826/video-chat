...
function Parent() {
    ...
    const sendMessage = (e) => {
        ...
        socket.emit("send-message", {
            senderId: userId,
            senderModel: "Parent", // Known for this page
            receiverId: selectedUser._id,
            receiverModel: selectedUser.role === "faculty" ? "Faculty" : (selectedUser.role === "student" ? "Student" : "Parent"),
            text: newMessage
        });
    };
    ...
    const startCameraPreview = async (type) => {
    ...
    const acceptCall = useCallback(async (roomName, type) => {
        try {
            ...
            let tracks = await createLocalTracks({ audio: true, video: (type === "video") });
            setLocalTracks(tracks);
    ...
    useEffect(() => {
        ...
        socket.on("call-response", ({ accepted, roomName, type }) => {
            if (accepted) acceptCall(roomName, type);
    ...
    return (
        <div className="h-screen bg-gray-100 flex shadow-2xl overflow-hidden antialiased text-gray-900 overflow-hidden">
                                ...
                                <div className="flex items-center gap-6">
                                    ...
                                    <div>
                                        ...
                                        <div className="flex items-center gap-3 text-white/50 text-[10px] font-black uppercase tracking-[0.2em]">
                                            ...
                                            {callType === "voice" ? "SECURE VOICE CONNECTION" : "SECURE VIDEO CONNECTION"}
                                        </div>
                                    </div>
                                </div>
                                <div className="flex items-center gap-4">
                ...
                <div className={`fixed z-[150] transition-all duration-500 ease-in-out group/call ${isMinimized
                    ...
                    {isMinimized && (
                        <div className="absolute bottom-5 left-5 right-5 flex items-center justify-between z-[170] pointer-events-none transition-opacity duration-300 group-hover/call:opacity-0">
                            <div className="flex items-center gap-4 text-white">
                                {callType === "voice" ? (
                                    <div className="w-10 h-10 rounded-full bg-blue-600 flex items-center justify-center font-black animate-pulse">
                                ...
                                <div>
                                    <div className="text-xs font-black uppercase tracking-widest">{callType === "voice" ? "Voice Call" : "Video Call"}</div>
                                    <div className="text-[10px] uppercase font-bold text-white/40">{selectedUser.username}</div>
                                </div>
                            </div>
                        </div>
                    )}
                    ...
                    {isMinimized && (
                        <div className="absolute inset-0 bg-black/40 backdrop-blur-sm opacity-0 group-hover/call:opacity-100 transition-all duration-300 flex items-center justify-center gap-4 z-[165]">
                            ...
                            {callType === "video" && (
                                <button onClick={toggleVideo} className={`w-14 h-14 rounded-2xl flex items-center justify-center transition-all ${isVideoOff ? "bg-red-500 text-white" : "bg-white/10 text-white hover:bg-white/20"}`}>
                        ...
                        <div className={`flex-grow relative bg-gray-900 overflow-hidden border border-white/5 transition-all duration-700 ${isMinimized ? "rounded-none h-full" : "rounded-[4rem] shadow-[0_0_120px_rgba(0,0,0,0.8)]"}`}>
                            ...
                            {!isMinimized && (
                                <>
                                    ...
                                    <div className="absolute top-12 right-12 flex items-center gap-3 bg-blue-600/30 backdrop-blur-2xl px-5 py-3 rounded-2xl border border-blue-500/30 text-blue-300 text-[10px] font-black uppercase tracking-[0.3em]">
            ...
            {incomingCall && (
                <div className="fixed inset-0 z-[100] flex items-center justify-center bg-blue-900/40 backdrop-blur-xl p-6">
                    <div className="bg-white p-10 rounded-[3.5rem] border border-white w-full max-w-md text-center shadow-[0_40px_100px_rgba(30,58,138,0.3)] transform scale-110">
                        ...
                        <h2 className="text-3xl font-black text-gray-800 mt-6">{incomingCall.name}</h2>
                        <p className="text-gray-400 text-xs font-bold uppercase tracking-widest mt-2">Incoming {incomingCall.type === "voice" ? "Voice" : "Video"} Call</p>
